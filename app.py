...
st.markdown(
    ...
    .hero h1 {
        font-size: 2.6rem; font-weight: 700; margin-bottom: 0.5rem; color: #fff;
    }
    .hero p {
        font-size: 1.1rem; color: #d0d0d0; margin-top: 0.3rem;
    }
    .hero img {
        margin-top: 1.5rem; border-radius: 10px; width: 60%; max-width: 700px;
        box-shadow: 0 4px 16px rgba(0,0,0,0.5);
    }
    .metric-container { display:flex; justify-content:center; gap:3rem; margin-top:2rem; }
    .metric { text-align:center; }
    .metric h3 { font-size:1rem; color:#aaa; margin-bottom:0.4rem; }
    .metric p  { font-size:1.4rem; font-weight:600; color:#fff; }
    </style>
    ...
    <div class="hero">
        ...
        <div class="metric-container">
            <div class="metric"><h3>Model</h3><p>Gradient Boosting</p></div>
            <div class="metric"><h3>Mean Abs. Error</h3><p>≈ 6.38 cycles</p></div>
            <div class="metric"><h3>Dataset</h3><p>NASA CMAPSS FD001</p></div>
        </div>
    </div>
    """,
...
def ensure_features(df: pd.DataFrame, auto_fe: bool) -> pd.DataFrame:
    ...
    if looks_like_raw(df) and not auto_fe:
        st.warning("Detected RAW CMAPSS schema but Auto-Feature is OFF. Enable the toggle in the sidebar.")
        st.stop()
...
with st.sidebar:
    st.header("Settings")
    auto_fe = st.toggle(
        "Auto feature engineering (for RAW input)",
        value=True,
